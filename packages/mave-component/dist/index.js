...
var MaveComponent = class extends LitElement2 {
  ...
  play() {
    var _a, _b, _c;
    if (this.video && ((_a = this.video) == null ? void 0 : _a.currentTime) >= ((_b = this.video) == null ? void 0 : _b.duration))
      this.video.currentTime = 0;
    (_c = this.video) == null ? void 0 : _c.play();
  }
  pause() {
    var _a;
  ...
  generateStyle() {
    ...
    if (this.width && this.height) {
      ...
      if (this.aspectRatio) {
        ...
        css3.textContent = `:host { display: block; overflow: hidden; aspect-ratio: ${aspectRatio}; width: 100%; min-width: 320px; min-height: 180px; } canvas, .poster { aspect-ratio: ${aspectRatio}; object-fit: contain; } #video, #video[poster] { object-fit: contain; }`;
      } else {
  ...
  videoRendered(video) {
    videoEvents.forEach((type) => {
      video == null ? void 0 : video.addEventListener(type, (event) => {
        ...
        if (type == "seeked") {
          data = {
            detail: { duration: (_a = this.video) == null ? void 0 : _a.duration, embed: this.embed }
          };
        } else {
          data = { detail: { embed: this.embed } };
        }
        this.dispatchEvent(new CustomEvent(event.type, data));
      });
    });
  }
  render() {
  ...
  sendMessage(event, options = {}) {
    if (!this.iframe.contentWindow || !this.video)
      return;
    const payload = __spreadProps(__spreadValues({ message: event }, options), { embed: this.embed });
...
__decorateClass([
  property2({ type: String, attribute: "file-type" })
...
__decorateClass([
  query2("#end")
], MaveComponent.prototype, "endElement", 2);
__decorateClass([
  state2()
